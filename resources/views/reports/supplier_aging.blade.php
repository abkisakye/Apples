@extends('layouts.app', ['title' => 'Supplier Aging'])
...
@section('content')
    ...
    <div class="page-head">
        <div>
            ...
            <p>Outstanding supplier balances grouped into aging buckets based on due dates.</p>
        </div>
        <div class="actions">
            <a href="{{ route('reports.supplier-aging.export') }}" class="button-link">Export CSV</a>
            <a href="{{ route('reports.customer-aging') }}" class="button-link">Customer Aging</a>
            @if ($access->can('supplier_payments.manage'))
                <a href="{{ route('supplier-payments.index', ['period' => 'today']) }}" class="button-link primary">Today Supplier Payments</a>
            @endif
        </div>
    </div>
    ...
    <section class="panel">
        <p class="list-note">Use this report before supplier follow-up or payment posting so the oldest balances are handled first.</p>
        ...
        <table>
            ...
            <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="cell-stack">
                                <div class="table-title"><a href="{{ route('suppliers.statement', $row->supplier) }}">{{ $row->supplier->name }}</a></div>
                                <div class="status-inline">
                                    <span class="badge {{ $row->days_90_plus > 0 ? 'credit' : 'success' }}">
                                        {{ $row->days_90_plus > 0 ? '90+ overdue' : 'Active supplier account' }}
                                    </span>
                                    @if ($row->days_1_30 > 0)
                        ...
                        <td>
                            <div class="action-stack">
                                <a href="{{ route('suppliers.statement', $row->supplier) }}" class="action-chip">Statement</a>
                                @if ($access->can('supplier_payments.manage'))
                                    <a href="{{ route('supplier-payments.create', ['supplier_id' => $row->supplier->id]) }}" class="action-chip primary">Record Payment</a>
                                @endif
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="muted">No outstanding supplier accounts were found for this view.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
